export class DiscountService {
   ...
   */
  static async createShopifyVolumeDiscount({admin, fetcherData, discountFunctionId}) {
    const metafieldsValue = {
      ...
      discounts: DiscountService.structureDiscountData(fetcherData),
   ...
   * @throws {Error} When GraphQL request fails
   */
  static async updateShopifyVolumeDiscount({admin, fetcherData, metafieldId}) {
    const metafieldsValue = {
      targetMode: fetcherData.whichProducts,
      campaignName: fetcherData.name,
      selectedProductIds: JSON.parse(fetcherData.selectedProducts).map(p => p.id),
   ...
   */
  static async deleteShopifyVolumeDiscount(admin, shopifyDiscountId) {
    const response = await admin.graphql(
      `#graphql
   ...
   */
  static structureDiscountData(fetcherData) {
    let discountData = [];
   ...
   */
  static async getDiscountMetafieldId({admin, shopifyDiscountId}) {
    const response = await admin.graphql(
      `#graphql
      query {
